...
import { Upload, X, Camera, QrCode, Facebook, Instagram, Linkedin, Youtube, Twitter, MessageCircle } from 'lucide-react';
...
const PRESET_LOGOS = [
  ...
  { id: 'whatsapp', type: 'svg', label: 'WhatsApp' },
  { id: 'instagram', type: 'icon', icon: Instagram, label: 'Instagram' },
  { id: 'facebook', type: 'icon', icon: Facebook, label: 'Facebook' },
  { id: 'youtube', type: 'icon', icon: Youtube, label: 'YouTube' },
  { id: 'linkedin', type: 'icon', icon: Linkedin, label: 'LinkedIn' },
  { id: 'twitter', type: 'icon', icon: Twitter, label: 'Twitter' },
...
const LogoTab: React.FC<LogoTabProps> = ({ styling, onStyleChange }) => {
  ...
  const handlePresetSelect = (preset: typeof PRESET_LOGOS[0]) => {
    ...
    } else if (preset.type === 'icon' || preset.type === 'text' || preset.type === 'svg') {
  ...
  const generatePresetSvg = (preset: typeof PRESET_LOGOS[0]): string => {
    ...
    if (preset.type === 'icon' && preset.id === 'camera') {
      ...
      `;
    } else if (preset.type === 'svg' && preset.id === 'whatsapp') {
      svgContent = `
      ...
      `;
    } else if (preset.type === 'icon' && preset.id === 'instagram') {
      svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
          <rect width="24" height="24" fill="white"/>
          <defs>
      ...
      `;
    } else if (preset.type === 'icon' && preset.id === 'facebook') {
      svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
          <rect width="24" height="24" fill="white"/>
      ...
      `;
    } else if (preset.type === 'icon' && preset.id === 'youtube') {
      svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
          <rect width="24" height="24" fill="white"/>
      ...
      `;
    } else if (preset.type === 'icon' && preset.id === 'linkedin') {
      svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
          <rect width="24" height="24" fill="white"/>
          <rect x="1" y="1" width="22" height="22" rx="3" fill="#0A66C2"/>
          <path fill="white" d="M7.5 9.5v7h-2v-7h2zm-1-3.5a1.25 1.25 0 1 1 0 2.5 1.25 1.25 0 0 1 0-2.5zm9 3.5c2.21 0 4 1.79 4 4v3.5h-2v-3.5a2 2 0 0 0-4 0v3.5h-2v-7h2v1a3 3 0 0 1 2-1z"/>
        </svg>
      `;
    } else if (preset.type === 'icon' && preset.id === 'twitter') {
      svgContent = `
        <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24">
          <rect width="24" height="24" fill="white"/>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
          ...
          {PRESET_LOGOS.map((preset) => (
            ...
            >
              ...
              {preset.type === 'svg' && preset.id === 'whatsapp' && (
